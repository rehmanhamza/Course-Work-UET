...
int main() {
    ...
    else
    {
        for(int a=0;a<num_of_code;a++)
        {
            ifstream checker;
            char* file_na;
            file_na=(char *)malloc(sizeof(char)*10000);
            cout<<"Enter the filename or directory "<<(a+1)<<" : ";
            cin>>file_na;
            checker.open(file_na);
    ...
    for(int tr=0;tr<num_of_code;tr++)
    {
        int comment_remover=0;
        for(int a=0;code1[tr][a]!='\0';a++)
        {
            if(47==(int)code1[tr][a] && 42==(int)code1[tr][a+1])
            {
                while(true)
                {
                    a++;
                    if(47==(int)code1[tr][a+1] && 42==(int)code1[tr][a])
                    {
                        a=a+1;
                        break;
                    }
            ...
            {
                code1_withoutspace[tr][comment_remover]=code1[tr][a];
                comment_remover++;
            }
        }
        code1_withoutspace[tr][comment_remover]='\0';
    }
    for(int a=0;a<num_of_code;a++)
    {
